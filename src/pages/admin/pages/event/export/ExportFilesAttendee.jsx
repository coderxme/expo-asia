...
export default function ExportFilesAttendee({ eventAttendeeData, totalOfAttendee }) {

  const generateXLSXData = () => {
    const worksheet = XLSX.utils.json_to_sheet(generateCSVData());
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Event Attendee Data');
  ...
  const generateCSVData = () => {
    const csvData = eventAttendeeData.map(item => ({
      DateTime: item.created_at ? formatDate(item.created_at) : "N/A",
      Name: item?.participant_details?.name || "N/A",
      Email: item?.participant_details?.name || "N/A",
      PhoneNumber: item?.participant_details.phone_no || "N/A",
      Company: item?.participant_details?.company_org_details?.name || "N/A",
    }));
    return csvData;
  };
    ...
    <Document>
      <Page size="A4" style={styles.page} orientation='landscape'>
        <View style={styles.section}>
          ...
          <Text style={styles.headerText}>List of Event Attendee </Text>
          </View>
          <View>
          ...
          {eventAttendeeData.map((item, index) => (
            <View key={index} style={styles.tableRow}>
              ...
              <Text style={styles.tableCell}>{item?.participant_details?.company_org_details?.name || "N/A"}</Text>
            </View>
          ))}
        </View>
  ...
  return (
        ...
        <Option value="PDF">
            <PDFDownloadLink document={generatePDFData()} fileName="Event Attendee.pdf">
        ...
        <button onClick={() => {
            ...
            a.href = url;
            a.download = 'Event Attendee.xlsx';
            a.click();
